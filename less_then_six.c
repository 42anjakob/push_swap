...
/*   Updated: 2026-01-12 23:55:55 by anjakob          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	if_three(t_a_stack *a, size_t plus)
{
	if (a->stack[0] == 2 + plus && a->stack[1] == 1 + plus
		&& a->stack[2] == 0 + plus
		&& s(a->stack, a->len) && rr(a->stack, a->len))
		write(1, "sa\nrra\n", 7);
		...
		&& a->stack[2] == 2 + plus && s(a->stack, a->len))
		write(1, "sa\n", 3);
		...
		&& a->stack[2] == 1 + plus
		&& r(a->stack, a->len) && s(a->stack, a->len) && rr(a->stack, a->len))
		write(1, "ra\nsa\nrra\n", 10);
}

void	if_four(t_a_stack *a, t_b_stack *b)
{
	if (a->stack[0] == 0 && p(a->stack, b->stack, a->len, b->len))
	{
		write(1, "pb\n", 3);
		if_three(a->stack, 1);
...
void	if_five(t_a_stack *a, t_b_stack *b)
{
	...
	{
		if_three(a, 2);
		if (b->stack[0] == 0 && s(b->stack, b->len))
			write(1, "sb\n", 3);
	}
	else if (r(a->stack, a->len))
		write(1, "ra\n", 3);
